...
// USHRT_MAX: unsigned short int max value (port numbers usually lie in this range)
...
{
	printf("Usage: Firewall Rule Options:\n"
	       "Firewall implements an exact match algorithm, where "
...
/*
 * The function sends a command to a Firewall module via a device file.
 */
...
static void view_rules(void)
{
	...
	// iteratively calls fw_dev_read from kernel module
	// inet_ntoa -> binary form of IP address to readable string
	while((byte_count = fread(buffer, 1, sizeof(struct fw_rule), fp)) > 0) 
	{
		rule = (struct fw_rule *)buffer;
...
static int parse_arguments(int argc, char **argv, struct fw_ctl *ret_ctl)
{
	...
	// NOTE: for options that require an argument, 'optarg' stores the argument that succeeds
	// the command
	// Eg: For s_ip, optarg will store source IP Address
	
	while(1) 
	{
		opt_index = 0;

		// processes command line arguments one by one. When all are processed, -1 returned
		// getopt_long chooses among the options defined in long_options[]
		...
		switch(opt) 
		{
			...
			case 's':	/* Source ip address */
				// inet_aton -> IP address from string to binary form
	...
	if(ctl.mode != FW_VIEW && ctl.rule.in == -1) 
	{
